...
<script>
	function pay_update(order_code,flag){
		...
		var order_code = order_code;
 		...
 		var BUYER_NUMBER = '${BUYER_NUMBER}';
 		console.log(BUYER_EMAIL);
 		console.log(BUYER_NUMBER);
 	if(confirm("정말 주문취소 하시겠습니까?") == true){
 		$.ajax({
 			url: "/MODA/noorderlist/payUpdate",
 			data : {"ORDER_CODE": order_code, "flag":en_flag, "BUYER_EMAIL":BUYER_EMAIL, "BUYER_NUMBER":BUYER_NUMBER},
 			type: "get",
 			async:true,
...
if(confirm("구매확정 하시겠습니까?") == true){
	$.ajax({
		url: "/MODA/noorderlist/payUpdate",
		data : {"currentPage":currentPage, "ORDER_CODE": order_code, "flag":en_flag, "BUYER_EMAIL":BUYER_EMAIL, "BUYER_NUMBER":BUYER_NUMBER},
		type: "get",
		async:true,
		success : function(data){
			$("#account-contentsWrap").html(data);
...
function paging(num){
	   ...
	   console.log(BUYER_NUMBER);
	   $.ajax({
	      url: "/MODA/noMemberOrderList",
	      type: "post",
	      data: {"currentPage":currentPage, "BUYER_EMAIL":BUYER_EMAIL, "BUYER_NUMBER":BUYER_NUMBER, "paging":paging},
	      success:function(data){
	         $("#account-contentsWrap").html(data);
	         console.log(data);
...
<!-- <h3 style="margin:20px 0;">비회원 주문내역</h3> -->
	...
	<!-- my-account//end -->
	<br/><br/><br/><br/><br/><br/>
	<div class="account-contentsWrap" id="account-contentsWrap">
