						...
						<table class="table" id="tb01">
							...
							<thead>
								<tr>
									...
									<th scope="col" class="situation">교환 신청</th>
								</tr>
							</thead>
							<tbody>
								<c:choose>
							...
							<c:forEach items="${list }" var="order">
									...
									<td class="delivery"><div style='display: none;'>${order.ORDER_CODE}</div>${order.ORDER_DELIVERY_STATE }</td>
									<td class="situation"><div style='display: none;'>${order.ORDER_CODE}</div>${order.ORDER_PAY_STATE }</td>
									<c:choose>
									<c:when test="${order.ORDER_STATE eq '교환신청'}">
									...
									<a class="oto_aaaa" target="modal" data-size="md" data-label="교환신청" href="/MODA/exchangelist/form?ORDER_CODE=${order.ORDER_CODE}">
									<span>교환신청</span>
									</a>
									</div>
									</td>
